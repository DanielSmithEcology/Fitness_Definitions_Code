...
Annual.Model.Comp <- function(N1,N2,s,g1,g2,f,a){
  ...
  EMER1 <-  rbinom(1,GER1,1/(1+(GER1+GER2)*a))
  EMER2 <-  rbinom(1,GER2,1/(1+(GER1+GER2)*a))
...
for(xx in 1:gI.Sims2b){ # for all gI 
  ...
  N_I[1] <- 2 # initial abundance (set to 2 to reduce noisiness; =1 makes no qualitative change)
...
gI.Sims   <- length(gI.Vecb) # number of sims 
...
for(xx in 1:gI.Sims){ # for timesteps 
  gI <- gI.Vecb[xx] # set gI value 
  GeoMean <- rep(NA,Time.Sim-1)
...
myPlot <- function(x,y, yaxt = "n", ylab = NA) {
  plot(x, y, yaxt = yaxt,  pch = 19, cex=1, col="blue", bg="blue", lwd=2,type="p",xlim=c(0,1),cex.axis=1.5,ylim=c(.015,.86),las=1,ylab="",xlab="",xaxt="n")
  axis(2,at=c(0,.4,.8),cex.axis=1.5,las=1,col.axis = "blue")
...
par(mar = c(6, 5, 5, 4))
# plot 1
plot(gI.INVX~gI.Vec, pch = 19,ylab="",xlab="", cex=1, type = "p",bg="black",col="darkgreen",xlim=c(0,1),cex.axis=1.5,ylim=c(gI.INVX[1],max(gI.INVX)*1.5),las=1,col.axis = 'darkgreen',xaxt="n",yaxt="n")
axis(1,at=c(0,.5,1),col.axis = 'black',cex.axis=1.5)
axis(4,at=c(0,.1,.2),cex.axis=1.5,las=1,col.axis = "darkgreen")
par(new=T)
...
par(mar = c(6, 5, 5, 4))
plot(gI.INV~gI.Vecb, pch = 19,ylab="",xlab="", cex=1, type = "p",bg="black",col="darkgreen",xlim=c(0,1),cex.axis=1.5,ylim=c(gI.INV[1],max(gI.INV)*1.05),las=1,col.axis = 'darkgreen',xaxt="n",yaxt="n")
axis(1,at=c(0,.5,1),col.axis = 'black',cex.axis=1.5)
axis(4,at=c(0,.3,.6),col.axis = 'darkgreen',cex.axis=1.5,las=1)
par(new=T)
